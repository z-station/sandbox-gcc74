...
def test_execute__float_result__ok():
    ...
    file = CppFile(code)
    CppService._compile(file)

    # act
    exec_result = CppService._execute(
        data_in=data_in,
        file=file
    )
...
def test_execute__timeout__return_error(mocker):
    ...
    # act
    execute_result = CppService._execute(file=file)
...
def test_execute__write_access__error():
    ...
    # act
    exec_result = CppService._execute(file=file)
...
def test_execute__proc_exception__raise_exception(mocker):
    ...
    with pytest.raises(exceptions.ExecutionException) as ex:
        CppService._execute(file=file, data_in=data_in)
...
def test_compile__timeout__error(mocker):
    ...
    mocker.patch.object(CppFile, '__new__', return_value=file_mock)
    ...
    # act
    error = CppService._compile(file_mock)
...
def test_check__true__ok():
    ...
    # act
    check_result = CppService._check(
...
def test_debug__compile_is_success__ok(mocker):
    ...
    compile_mock = mocker.patch(
        'app.service.main.CppService._compile',
        return_value=None
    )
    ...
    execute_mock = mocker.patch(
        'app.service.main.CppService._execute',
    ...
    # act
    debug_result = CppService.debug(data)
...
def test_debug__compile_return_error__ok(mocker):
    ...
    execute_mock = mocker.patch('app.service.main.CppService._execute')
    data = DebugData(
        code='some code',
        data_in='some data_in'
    )
...
def test_testing__compile_is_success__ok(mocker):
    ...
    check_mock = mocker.patch(
        'app.service.main.CppService._check',
    ...
    # act
    testing_result = CppService.testing(data)
...
def test_testing__compile_return_error__ok(mocker):
    ...
    execute_mock = mocker.patch('app.service.main.CppService._execute')
    check_mock = mocker.patch('app.service.main.CppService._check')
    test_1 = TestData(
        data_in='some test input 1',
        data_out='some test out 1'
    )
